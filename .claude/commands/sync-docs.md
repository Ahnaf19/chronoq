...
1. **API endpoints** — Read `chronoq_server/chronoq_server/api/tasks.py` and `api/metrics.py`. Compare the actual routes, request/response models, and query params against `docs/api-reference.md` and `README.md` API table. Flag any mismatches.

2. **Configuration** — Read `chronoq_server/chronoq_server/config.py` and `chronoq_predictor/chronoq_predictor/config.py`. Compare actual env vars and config fields against `docs/configuration.md` and `.env.example`. Flag any mismatches.
