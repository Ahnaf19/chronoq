...
- **Auto-retrain**: triggered when `_records_since_last_retrain >= config.retrain_every_n`. Uses an in-process monotonic counter on `TaskRanker` rather than `store.count_since(_last_retrain_at)`, because Windows `datetime.now()` has ~15.6ms resolution (`GetSystemTimeAsFileTime`) — rapid `record()` calls after a retrain can share `recorded_at` with `_last_retrain_at` and be missed by the strict-`>` comparison. The counter increments on every `record()` and resets after every `retrain()` attempt (promoted or not). `_last_retrain_at` is still stamped for logging and drift diagnostics. The `TelemetryStore.count_since` method is preserved for introspection but is no longer load-bearing for the retrain decision. **Precision caveat for `count_since`**: on Windows, records saved inside the same 15ms tick as the cutoff datetime may be undercounted — this is a fundamental `datetime.now()` resolution limit, not a code bug. Introspection use tolerates up to ~15ms of undercount; tests that exercise count_since across a retrain boundary must `time.sleep(0.025)` to advance past a tick.
- **Model versioning**: `LambdaRankEstimator.version()` embeds a per-instance monotonic counter (`_version_counter`) alongside the wall-clock timestamp (e.g. `lambdarank-v20260424033012-0003`). This keeps versions strictly ordered and unique even when multiple incremental fits complete within one Windows 15ms clock tick — needed for drift-recovery runs and any tight retrain loop. `HeuristicEstimator` uses a similar counter (`heuristic-v{n}`).
- **Heuristic fallback**: retained for cold start and unseen `task_type`s.
- **Drift detection**: `drift_status()` returns `DriftReport | None`. After a successful lambdarank full fit, `DriftDetector.set_reference()` is called; subsequent retrains check drift (non-blocking — logged, not gate-blocking). **Semantics note**: the PSI comparison is *inter-retrain* — current training batch vs the previous retrain's training batch. It detects workload-shift between retrain cycles, NOT model-fit quality within a single retrain. In tight retrain loops (e.g. the drift-recovery bench experiment), the report can read `status=stable` on cycles 2+ even under a shifted distribution because `set_reference()` was just updated to the shifted data on cycle 1. This is expected.
